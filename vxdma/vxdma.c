...
static void xilinx_dma_halt(struct xilinx_dma_chan *chan)
{
	...
	/*[vutt6] Set bit XILINX_DMA_CR_RUNSTOP_MASK to start axiDma*/
...
static void xilinx_dma_start_transfer(struct xilinx_dma_chan *chan)
{
	...
	if (chan->err) {
		printk("%s: Channel error\n", __FUNCTION__);
		return;
	}

	if (list_empty(&chan->pending_list)) {
		printk("%s: pending_list empty\n", __FUNCTION__);
		return;
	}

	if (!chan->idle) {
		printk("%s: chan is not idle\n", __FUNCTION__);
		return;
	}
	...
	unsigned long flags)
{
	...
	if (!is_slave_direction(direction)) {
		printk("%s: direction invalid", __FUNCTION__);
		return NULL;
	}
	...
	desc = xilinx_dma_alloc_tx_descriptor(chan);
	if (!desc) {
		printk("%s: can not allocate tx desc", __FUNCTION__);
		return NULL;
	}
...
#ifdef CONFIG_PHYS_ADDR_T_64BIT
			hw->buf_addr = lower_32_bits(buf_addr + sg_used + (period_len * i));
			hw->buf_addr_msb = upper_32_bits(buf_addr + sg_used + (period_len * i));
#else
 ...
 * Return: '0' on success and failure value on error
 */
static int xilinx_dma_chan_probe(struct xilinx_dma_device *xdev, struct device_node *node) {
	...
	/*vutt6: add more channel infor for searching*/
...
// 	struct xilinx_dma_tx_descriptor *new_tx_desc;
// 	struct xilinx_dma_tx_segment *new_segment;
// 	struct dma_async_tx_descriptor *dma_async_tx_desc;

// 	dma_addr_t *src_dma_buffer = (dma_addr_t *) kmalloc(1024, GFP_KERNEL);
// 	if (src_dma_buffer == NULL) {
// 		printk("%s: can not allocate buffer\n", __FUNCTION__);
// 		return -1;
// 	}

// 	printk("%s: allocate new_tx_desc\n", __FUNCTION__);

// 	dma_async_tx_desc = xilinx_dma_prep_dma_cyclic(chan, src_dma_buffer, 1014, 1024,
// 							DMA_DEV_TO_MEM, 0);

// 	// new_tx_desc =	xilinx_dma_alloc_tx_descriptor(chan);

// 	if (dma_async_tx_desc == NULL) {
// 		printk("%s: cannot allocate new_tx_desc\n", __FUNCTION__);
// 		return -1;
// 	}
	
// 	// new_tx_desc->async_tx;

// 	// new_segment = xilinx_dma_alloc_tx_segment(chan);

// 	// printk("%s: append desc\n", __FUNCTION__);
// 	// append_desc_queue(chan, new_tx_desc);

// 	printk("%s: start transfer\n", __FUNCTION__);
// 	xilinx_dma_start_transfer(chan);
// 	return;
// }

static const struct of_device_id xilinx_dma_of_match[] = {
	{ .compatible = "vxlnx,axi-dma-1.00.a",},
	{}
};
