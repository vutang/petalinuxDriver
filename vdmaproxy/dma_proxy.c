...
static void transfer(struct dma_proxy_channel *pchannel_p)
{
	...
	// printk(KERN_INFO "%s.pchannel_p->dma_handle: 0x%x", (u32) pchannel_p->dma_handle);

	/* Start the DMA transfer and make sure there were not any errors
	 */
	printk(KERN_INFO "%s.Starting %s DMA transfers\n", __func__,
...
static long ioctl(struct file *file, unsigned int unused , unsigned long arg)
{
	...
	printk("%s.Channel %s is called\n", __func__, 
		(pchannel_p->direction == DMA_MEM_TO_DEV) ? "TX" : "RX");
	
	printk("%s.pchannel_p->interface_p->length: %d\n", __func__, pchannel_p->interface_p->length);
	if (pchannel_p->direction == DMA_MEM_TO_DEV) {
		for (i = 0; i < 4; i++)
			printk(KERN_INFO "%s.pchannel_p->interface_p->buffer[%d]: %d\n", __func__, i, 
				pchannel_p->interface_p->buffer[i]);
	}
...
bool xdma_filter_1(struct dma_chan *chan, void *param) {
	// printk("%s: is called\n", __func__);
...
static int create_channel(struct dma_proxy_channel *pchannel_p, char *name, u32 direction)
{
	 ...
	 */
	pchannel_p->channel_p = dma_request_channel(mask, xdma_filter_1, (void *) &match);
	if (!pchannel_p->channel_p) {
		dev_err(pchannel_p->dma_device_p, "DMA channel request error\n");
		return ERROR;
	}
	else {
		printk("%s.request DMA channel successfully\n", __func__);
	}
	...
	if (cached_buffers) {
		...
		printk(KERN_INFO "%s.pchannel_p->interface_phys_addr 0x%08X\n", __func__,
			   (unsigned int)pchannel_p->interface_phys_addr);
	}

	printk(KERN_INFO "%s.Allocating %d (bytes) cached memory at 0x%08X\n", __func__,
		(unsigned int) sizeof(struct dma_proxy_channel_interface), 
